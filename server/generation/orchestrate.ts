...
/**
 * Бюджет вывода с запасом на размышление модели: у Gemini 2.5 мысли считаются
 * в тот же лимит, что и текст ответа. При прежних 8192 включённое размышление
 * обрывало ответ на середине партии, и посты приходилось добирать повторными
 * запросами — то есть экономия на лимите оплачивалась лишними вызовами.
 */
const MAX_OUTPUT_TOKENS = 16_384;
/** Размышление добавляет секунды к ответу, поэтому и ожидание длиннее. */
