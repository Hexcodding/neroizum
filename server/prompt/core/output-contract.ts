...
Требования к postContent: обычный текст без Markdown, без звёздочек и решёток для выделения. Абзацы разделяй двойным переводом строки.

ЭМОДЗИ: от одного до трёх на пост — там, где они несут смысл: перед пунктами списка, в хуке, рядом с призывом. Не по одному в каждом абзаце и не гирляндой в конце. Совсем без эмодзи пост оставляй только тогда, когда этого требует голос бренда или стоп-слова.`;
