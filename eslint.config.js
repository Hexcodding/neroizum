...
export default tseslint.config(
  ...
  {
    // Разметка считается иначе, чем логика. Шестьдесят строк — правильная планка
    // для функции с ветвлениями, но форма с шестью полями честно занимает сто
    // строк разметки, и дробить её на «компоненты» ради счётчика значит делать
    // код хуже. Ограничение на ветвление (complexity) при этом не ослаблено:
    // именно оно ловит запутанные компоненты.
    files: ["src/**/*.tsx"],
    rules: {
  ...
  {
    files: ["*.config.{ts,js}", "eslint/**/*.js", "scripts/**/*.{mjs,ts}"],
